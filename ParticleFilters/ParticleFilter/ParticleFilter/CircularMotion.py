...
# Please note that landmark coordinates are given in the form (y, x): 
# not in the traditional (x, y) format.
#  In this assignment,
#  treat the world as an infinite grid:
#  the robot can move so that its x- or y-coordinates are larger in magnitude
#  than the world_size variable. 
...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        # distance of forward motion
        distance = motion[1];
        #tolerance for beta ,turning angle
        tolerance = 0.001
        # create a new copy and initialize noise parameters
        result = robot(self.length)
        result.set_noise(self.bearing_noise, self.steering_noise ,self.distance_noise)

        #apply noise
        steering_angle_2 = random.gauss(steering_angle, result.steering_noise);
        distance_2 = random.gauss(distance, result.distance_noise);

        beta = (distance_2/self.length) * tan(steering_angle_2);   
        ...
        if(beta >tolerance):
            ...
            R = distance_2/beta;
            # center coordinates
            cx = self.x - R * sin(self.orientation);#self.orientation is theta in the lecture video 
            ...
            result.x = self.x + distance_2 * cos(self.orientation);
            result.y = self.y + distance_2 * sin(self.orientation);
            result.orientation = (self.orientation + beta)%(2*pi);
 ...
 # --------
    ...
    def sense(self,add_noise = 1): #do not change the name of this function
        ...
        # atan2(delta(y) ,delta(x)) - orientation(or theta)
        # Landmark coordinates are given in (y, x) form and NOT in the traditional (x, y) format!
        for i in range(len(landmarks)):
                       landmark_x = landmarks[i][1] ;
                       landmark_y = landmarks[i][0] ;
                       bearing = atan2(landmark_y - self.y, landmark_x - self.x) - self.orientation;
...
## out. Our testing program provides its own code for testing your
## move function with randomized motion data.
print(' sense() method test cases')

# sense() method test cases:-
## TEST CASES:
